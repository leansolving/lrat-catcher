  ...
  if emitShared then
    ...
    let workerLines : List String :=
      ["#!/bin/bash",
       ...
       "# build the Base module first: lake build " ++ modPrefix ++ ".Base",
       "# — one build up front also avoids concurrent workers racing to build",
       "# shared dependencies).",
       ...
       "TOOLROOT=\"$PWD\"                     # package root (workers run lake from here)",
       "SCRATCH=\"${TMPDIR:-/tmp}\"",
       "# -----------------------",
       "",
       ...
       "# lake lean, not raw lean -R: only lake loads the library's precompiled",
       "# dynlibs; a raw lean invocation runs project code in the IR interpreter.",
       "( lake lean \"$SRC\" -- -o \"$SCRATCH/Chunk$J.olean\" > \"$SCRATCH/lean$J.out\" 2>&1; echo $? > \"$SCRATCH/lean$J.rc\" ) &",
       "LEANPID=$!",
       "",
