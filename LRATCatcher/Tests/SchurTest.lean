...
-- the matching upper bound ¬hasKSchurFreeColoring 4 45, giving S(4)=44, is the
-- worked cube-and-conquer example `examples/schur4cc/`, whose certificates are
-- regenerated by its script rather than shipped).
